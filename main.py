...
cur = conn.cursor()
cur.execute(f"{drop_table_if_exists(get_table_name())}")
cur.execute(f"{create_table(df_from_csv, get_table_name())}")
...
with open(csv_file) as csv_file_with_open:
    ...
    for row in csv_reader:
        ...
        cur.execute(f"{insert_into_values(df_from_csv, get_table_name())}",
        tuple(lis))
        conn.commit()

# print(pd.read_sql(f"select * from {get_table_name()}", con = conn))
